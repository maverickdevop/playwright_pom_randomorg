...
3. Тесты запускаются без (UI/Графической оболочки), чтобы можно было увидеть процесс выполнения тестов в `conftest.py` нужно поменять аргументы запуска браузера:
```
launch_args = {
   "headless": True
}
```
Заменить значение `headless` на `False`. Первоначальная реализация необходима для запуска тестов в Docker. Контейнерный запуск не имеет графической карты
